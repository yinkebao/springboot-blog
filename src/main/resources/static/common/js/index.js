...
function setSwiperImgs() {
  ...
  swipers.forEach(function (value) {
    $("#container").append(
        '<div class="swiper-slide" style="background: url(' + value.img
        + ') no-repeat;background-size: 100% 100%;background-position:center;">'
        +
...
//本周热点
function weeFireArticles() {
  $.ajax({
    url: "/system/article/listWeeFireArticles",
    success: function (data) {
      data.module.forEach(function (article) {
        var str = '<div class="lineIfo"><a href="/system/article/viewArticle?id='
            + article.id
            + '"  target="_blank"><span class="glyphicon glyphicon-hand-right" style="color: rgb(255, 140, 60);"></span> &nbsp; '
            + article.title + '</a></div>';
        $("#weeFireArticles").append(str);
      })
    }
  })
}
...
//所有文章top5
function listTopFiveArticles() {
  $.ajax({
    url: "/system/article/listTopFiveArticles",
    success: function (data) {
      var colors=["label-danger","label-warning","label-success","label-default","label-default"];
      data.module.forEach(function (article,index) {
        var str = '<div class="lineIfo"><a href="/system/article/viewArticle?id='
            + article.id
        ...
        $.ajax({
          ...
          , success: function (data) {
            ...
            data.module.list.forEach(function (article) {
              ...
              str += '<p>';
              str += '<span class="glyphicon glyphicon-tag" style="color: rgb(174, 188, 135); font-size: 14px;">Python</span> '
                  + article.title;
              str += '</p>';
              str += '<a href="/system/article/viewArticle?id=' + article.id;
              if (article.titlePage==null){
                str += '"  target="_blank"><img src="/common/img/article/default.jpg"></a>';
              } else {
                str += '"  target="_blank"><img src="'+article.titlePage+'"></a>';
              }
              ...
              str += '<span class="glyphicon glyphicon-calendar" style="color: rgb(162, 179, 191); font-size: 14px;"></span> '
                  + new Date(article.publishDate).getFullYear() + '-'
                  + new Date(article.publishDate).getUTCMonth() + '-'
                  + new Date(article.publishDate).getDay();
              str += '<span class="glyphicon glyphicon-eye-open" style="color: rgb(162, 179, 191); font-size: 14px;"></span> '
                  + article.collectTimes;
              str += '</div></div></div>';
              lis.push(str);
            });
          }
    ...
    random_lines.forEach(function (r, idx) {
      ...
      for (i = idx + 1; i < all_array.length; i++) {
        ...
        if (null !== e.x && null !== e.y) {
          ...
          dist < e.max && (e === current_point && dist >= e.max / 2
